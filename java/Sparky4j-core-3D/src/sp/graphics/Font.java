...
public class Font extends NativeClass {
	...
	private static native long jniCreate(Buffer buffer, int fontsize, float size);
	...
	public Texture getTexture() {
		if(cache == CacheMode.ON_CREATE) {
			...
			if(texture == null) texture = new Texture(native_GetTexture(handler));
			return texture;
		}else {
			return new Texture(native_GetTexture(handler));
		}
	}
